...
var filteredNums = filterObj(compose(isOdd, listProduct), nums);

var filteredNumsProducts = mapObj(listProduct, filteredNums);

let result = reduceObj(sum, 0, filteredNumsProducts);
...
function reduceObj(reducerFn, initialValue, o) {
  ...
  for (let key of keys) {
    result = reducerFn(result, o[key]); // reducerFn need two params
  }
  return result;
}
